...
# final_merged_name = merge_blocks[-1].get("merged_name") if merge_blocks else None
...
# final_merged_df = all_merged_results[final_merged_name].copy()

# model_state["combined_dataset"] = final_merged_df

# st.success(f"✅ All merge operations completed successfully! Final dataset: '{final_merged_name}'.")

# elif all_merged_results:

# last_result_name = list(all_merged_results.keys())[-1]

# final_merged_df = all_merged_results[last_result_name].copy()
...
# model_state["combined_dataset"] = None

# st.error("❌ No merged table could be produced. Please check your merge configuration and try again.")



# # --- Show preview or error ---

# if isinstance(final_merged_df, pd.DataFrame) and not final_merged_df.empty:

# st.markdown("#### 🐞 Debug Preview: Final Merged Table")

# st.dataframe(final_merged_df.head(20), use_container_width=True)

# else:

# st.error("❌ The final merged dataset is empty. Please check your merge configuration and try again.")
