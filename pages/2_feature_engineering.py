...
#To merge datasets for MVP
if is_mvp:
    # Only do this once per session/model
    if "mvp_merged_dataset" not in st.session_state:
        try:
            merged = loan_level_df.copy()
            if not bureau_df.empty:
                merged = pd.merge(merged, bureau_df, on="Application_ID", how="left")
            if not installments_df.empty:
                merged = pd.merge(merged, installments_df, on="Application_ID", how="left")
            # Rename any column named 'Timestamp' to 'Timestamp_x'
            merged.columns = [
                "Timestamp_x" if col == "Timestamp" else col
                for col in merged.columns
            ]
            # Remove duplicate columns (keep first occurrence)
            merged = merged.loc[:, ~merged.columns.duplicated()]
            st.session_state["mvp_merged_dataset"] = merged
        except Exception as e:
            st.error(f"Error merging datasets for MVP: {e}")
            st.session_state["mvp_merged_dataset"] = pd.DataFrame()
    ...
    if model_state.get("show_merge", False): # Check the show_merge state within the model state
        if is_mvp:
            ...
            for i, block in enumerate(merge_blocks):
                ...
                common_cols = list(set(left_cols) & set(right_cols) - {"Timestamp"})
...
if st.button("📊 Save Attributes"):
    if is_mvp:
        ...
        if not merged.empty:
            ...
            st.success(f"Saved attributes for '{active_model}' successfully!")
        else:
...
if all_files_exist:
    if st.button("Proceed to Model Development"):
        st.switch_page("pages/3_Model_Development.py")
