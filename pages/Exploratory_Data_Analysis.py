...
#     clear_data_registry()
#     st.session_state.data_registry_cleared = True  # Set a flag to indicate the directory has been cleared
...
#     else:
#         st.error(f"Default On-Us data file '{default_file_path}' not found. Please upload a file.")
...
#     default_file_path = os.path.join(DEFAULT_DATA_DIR, "bureau_data.csv")
#     if os.path.exists(default_file_path):
#         st.info(f"Loading default Bureau data from '{default_file_path}'")
#         bureau_data = load_data(bureau_data, feature_mapping)
#         if bureau_data is not None:
#             if 'Timestamp' in bureau_data.columns and not bureau_data['Timestamp'].isnull().all():
#                 display_bureau_insights(bureau_data, "Bureau (Default)", feature_mapping)
#                 save_dir = DATA_REGISTRY_DIR
#                 data_path = os.path.join(save_dir, "bureau_data.parquet")
#                 try:
#                     bureau_data.to_parquet(data_path, index=False)
...
#                 st.error("Bureau data loaded but 'Timestamp' column is missing, invalid, or empty. Cannot proceed with insights.")
#     else:
#         st.error(f"Default Bureau data file '{default_file_path}' not found. Please upload a file.")
